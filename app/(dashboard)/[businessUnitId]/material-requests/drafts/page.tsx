...
import { getMaterialRequests } from "@/lib/actions/material-request-actions"
  ...
  return (
    <div className="flex-1 space-y-4">
      <div className="flex items-center justify-between space-y-2">
        <h2 className="text-3xl font-bold tracking-tight">Draft Requests</h2>
      </div>
      
      <Suspense fallback={<MaterialRequestsSkeleton />}>
        <DraftRequestsContent businessUnitId={businessUnitId} />
      </Suspense>
    </div>
  )
}

async function DraftRequestsContent({ businessUnitId }: { businessUnitId: string }) {
  const session = await auth()
